...
raw_router_tvl AS (
    ...
    FROM
        `mainnet-bigq.public.router_liquidity_events` AS r
    LEFT JOIN chains_meta AS cm ON r.domain = cm.domainid
    LEFT JOIN tokens_meta AS tm ON (r.asset = tm.local)
    ORDER BY
        1 DESC
),
...
fees AS (
    ...
    FROM
        `y42_connext_y42_dev.transfers_mapped` AS t
...
router_bal_hist AS (
    ...
    FROM
        (
            ...
            FROM
                `mainnet-bigq.y42_connext_y42_dev.routers_assets_balance_hist` AS ab
            LEFT JOIN `mainnet-bigq.public.assets` AS a
                ON
                    ab.asset_domain = a.domain
                    AND ab.asset_canonical_id = a.canonical_id
            LEFT JOIN chains_meta AS cm ON ab.asset_domain = cm.domainid
            LEFT JOIN tokens_meta AS tm ON a.local = tm.local
        )
    WHERE
        rn = 1
),
...
final AS (
    ...
    FROM
        combined_router_tvl_fee AS ctv
    FULL OUTER JOIN router_bal_hist AS rbh
        ON
            ctv.date = rbh.date
