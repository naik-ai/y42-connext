...
combined_cln_router_tvl_fee AS (
    ...
    FROM
        cln_fees AS ff
    FULL OUTER JOIN cln_routers_tvl
...
final_combined_cln_router_tvl_fee AS (
    ...
    FROM combined_cln_router_tvl_fee
),
...
clean_final AS (
    SELECT
        ...
        CASE
            ...
            WHEN
                STARTS_WITH(COALESCE(ad.asset, f.asset), 'next')
                THEN REGEXP_REPLACE(COALESCE(ad.asset, f.asset), '^next', '')
    ...
    FROM
        all_dates AS ad
    LEFT JOIN final_combined_cln_router_tvl_fee AS f
        ON
            ad.date = f.date
            AND LOWER(ad.router) = LOWER(f.router)
            AND ad.chain = f.chain
            AND ad.asset = f.asset
    LEFT JOIN `mainnet-bigq.raw.dim_connext_routers_name` AS r ON LOWER(ad.router) = LOWER(r.router)
),
